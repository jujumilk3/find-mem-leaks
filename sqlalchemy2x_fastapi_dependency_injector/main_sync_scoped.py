...
def create_app() -> FastAPI:
    container = Container()
    container.config.db_url.from_value("sqlite:///fastapi_sync_scoped_di_2x.db")
...
def perform_random_crud_operations(
    ...
    scoped_session with dependency_injector 사용
    """
...
def run_server():
    ...
    uvicorn.run(app, host="127.0.0.1", port=8032, log_level="error")
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        if result:
            print(
                f"| sqlalchemy2x_fastapi_sync_scoped_di | 10000_operations | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result:
            print(
                f"| sqlalchemy2x_fastapi_sync_scoped_di | 100000_operations | ✅ | {_ + 1} | "
                f"{result['memory_diff']} | {result['initial_memory']} | "
                f"{result['final_memory']} | {result['num_operations']} | "
                f"{result['create_ratio']} | {result['read_ratio']} | "
                f"{result['update_ratio']} | {result['delete_ratio']} |"
    ...
    for _ in range(5):
        ...
        if result_create_heavy:
            print(
                f"| sqlalchemy2x_fastapi_sync_scoped_di | create_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_read_heavy:
            print(
                f"| sqlalchemy2x_fastapi_sync_scoped_di | read_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_update_heavy:
            print(
                f"| sqlalchemy2x_fastapi_sync_scoped_di | update_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_delete_heavy:
            print(
                f"| sqlalchemy2x_fastapi_sync_scoped_di | delete_heavy | ✅ | {_ + 1} | "
