...
class Database:
    def __init__(self, db_url: str) -> None:
        ...
        async_session_factory = async_sessionmaker(self._engine)
    ...
    async def close(self) -> None:
        await self._async_scoped_session.remove()
...
async def get_async_scoped_db(database: Database = Depends(Provide[Container.database])):
    async with database.async_scoped_session() as session:
        yield session


def create_app() -> FastAPI:
    container = Container()
    container.config.db_url.from_value("sqlite+aiosqlite:///fastapi_async_scoped_di_2x.db")
    ...
    async def read_posts(db=Depends(get_async_scoped_db)):
        count_result = await db.scalar(select(Post.id).order_by(Post.id.desc()).limit(1))
        if count_result:
...
async def perform_random_crud_operations(
    ...
    async_scoped_session with dependency_injector 사용
    """
...
def run_server():
    ...
    uvicorn.run(app, host="127.0.0.1", port=8033, log_level="error")
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        if result:
            print(
                f"| sqlalchemy2x_fastapi_async_scoped_di | 10000_operations | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result:
            print(
                f"| sqlalchemy2x_fastapi_async_scoped_di | 100000_operations | ✅ | {_ + 1} | "
                f"{result['memory_diff']} | {result['initial_memory']} | "
                f"{result['final_memory']} | {result['num_operations']} | "
                f"{result['create_ratio']} | {result['read_ratio']} | "
                f"{result['update_ratio']} | {result['delete_ratio']} |"
    ...
    for _ in range(5):
        ...
        if result_create_heavy:
            print(
                f"| sqlalchemy2x_fastapi_async_scoped_di | create_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_read_heavy:
            print(
                f"| sqlalchemy2x_fastapi_async_scoped_di | read_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_update_heavy:
            print(
                f"| sqlalchemy2x_fastapi_async_scoped_di | update_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_delete_heavy:
            print(
                f"| sqlalchemy2x_fastapi_async_scoped_di | delete_heavy | ✅ | {_ + 1} | "
