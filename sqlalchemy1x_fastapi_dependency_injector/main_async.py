...
class Database:
    def __init__(self, db_url: str) -> None:
        ...
        self._session_factory = sessionmaker(self._engine, class_=AsyncSession)

    async def create_database(self) -> None:
        sync_engine = create_engine("sqlite:///fastapi_async_di_1x.db", connect_args={"check_same_thread": False})
...
async def get_async_db(database: Database = Depends(Provide[Container.database])):
...
def create_app() -> FastAPI:
    container = Container()
    container.config.db_url.from_value("sqlite+aiosqlite:///fastapi_async_di_1x.db")
    ...
    async def create_tables():
        await app.container.database().create_database()
    
    @app.post("/posts/")
    @inject
    async def create_post(title: str, body: str, db=Depends(get_async_db)):
        try:
    ...
    @app.get("/posts/")
    @inject
    async def read_posts(db=Depends(get_async_db)):
        try:
    ...
    @inject
    async def update_post(post_id: int, title: str, body: str, db=Depends(get_async_db)):
        try:
            await db.execute(
    ...
    @inject
    async def delete_post(post_id: int, db=Depends(get_async_db)):
        try:
    ...
    async def cleanup():
        """애플리케이션 종료 시 database 정리"""
        await app.container.database().close()
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        if result:
            print(
                f"| sqlalchemy1x_fastapi_async_di | 10000_operations | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result:
            print(
                f"| sqlalchemy1x_fastapi_async_di | 100000_operations | ✅ | {_ + 1} | "
                f"{result['memory_diff']} | {result['initial_memory']} | "
                f"{result['final_memory']} | {result['num_operations']} | "
                f"{result['create_ratio']} | {result['read_ratio']} | "
                f"{result['update_ratio']} | {result['delete_ratio']} |"
    ...
    for _ in range(5):
        ...
        if result_create_heavy:
            print(
                f"| sqlalchemy1x_fastapi_async_di | create_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_read_heavy:
            print(
                f"| sqlalchemy1x_fastapi_async_di | read_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_update_heavy:
            print(
                f"| sqlalchemy1x_fastapi_async_di | update_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        if result_delete_heavy:
            print(
                f"| sqlalchemy1x_fastapi_async_di | delete_heavy | ✅ | {_ + 1} | "
