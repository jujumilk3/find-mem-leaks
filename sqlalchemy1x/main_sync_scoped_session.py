...
def perform_random_crud_operations(
    ...
    sync_scoped_session 사용

    Args:
        sync_session: sync_scoped_session 객체
    ...
    for i in tqdm(range(num_operations)):
        ...
        if operation == "create":
            ...
            result = sync_session.execute("SELECT COUNT(*) FROM posts")
            count = result.scalar()
            if count > 0:
                post_id = random.randint(1, max(1, count))
                result = sync_session.execute(f"SELECT * FROM posts WHERE id = {post_id}")
                post = result.first()
                if post:
                    comments_result = sync_session.execute(f"SELECT * FROM comments WHERE post_id = {post_id}")
            ...
            if count > 0:
                post_id = random.randint(1, max(1, count))
                sync_session.execute(f"DELETE FROM posts WHERE id = {post_id}")
                sync_session.commit()

        # scoped_session의 특징: 주기적으로 세션 정리
        if (i + 1) % 1000 == 0:
            sync_session.remove()  # thread-local storage에서 세션 제거
            if DEBUG:
                print(f"Completed {i + 1} operations - session removed")
...
def main(
    ...
    engine = create_engine("sqlite:///test_sync_scoped.db", echo=False)

    Base.metadata.create_all(engine)

    # scoped_session 생성 - thread-local storage 사용
    session_factory = sessionmaker(bind=engine)
    scoped_session_obj = scoped_session(session_factory)
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | 10000_operations | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | 100000_operations | ✅ | {_ + 1} | "
            f"{result['memory_diff']} | {result['initial_memory']} | "
            f"{result['final_memory']} | {result['num_operations']} | "
            f"{result['create_ratio']} | {result['read_ratio']} | "
            f"{result['update_ratio']} | {result['delete_ratio']} |"
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | 1000000_operations | ✅ | {_ + 1} | "
            f"{result['memory_diff']} | {result['initial_memory']} | "
            f"{result['final_memory']} | {result['num_operations']} | "
            f"{result['create_ratio']} | {result['read_ratio']} | "
            f"{result['update_ratio']} | {result['delete_ratio']} |"
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | create_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | read_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | update_heavy | ✅ | {_ + 1} | "
    ...
    for _ in range(5):
        ...
        print(
            f"| sqlalchemy1x_sync_scoped_session | delete_heavy | ✅ | {_ + 1} | "
